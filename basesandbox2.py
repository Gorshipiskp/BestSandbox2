...
SUBS_CHARS = numpy.array(tuple(tuple(val for val in sub.values()) for sub in subs.values()))
...
class Matrix:
    ...
    def reset_field(self):
        ...
        self.temp_pmatrix = numpy.full(self.size, DEFAULT_TEMPERATURE, dtype=float)
    ...
    def changing_temp(pmatrix: numpy.ndarray, temp_pmatrix: numpy.ndarray,
        ...
        if not SUBS_CHARS[SUBS[pmatrix[x0 + x1, y0 + y1]], 3]:
            return
    ...
    def temp_iter(pmatrix, temp_pmatrix, pmatrix_bools, heat_coef):
        ...
        for x in prange(temp_pmatrix.shape[0]):
            for y in prange(temp_pmatrix.shape[1]):
                if SUBS_CHARS[SUBS[pmatrix[x, y]], 3]:
            ...
            for y in prange(shape[1]):
                ...
                if pixtypes[0]:
                    ...
                    if can_do:
                        ...
                        temp_pmatrix[x, y], temp_pmatrix[x_c, y] = temp_pmatrix[x_c, y], temp_pmatrix[x, y]
